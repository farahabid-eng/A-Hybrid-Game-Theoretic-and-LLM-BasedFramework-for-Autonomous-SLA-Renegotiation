...
You are an expert SLA analyst. Your task is to analyze raw stakeholder input and construct a \
structured stakeholder profile, as performed by the Profile Generation step of the SLA \
...
3. The stakeholder's acceptable value range per metric — the target and minimum acceptable \
...
When the counterparty presents a proposal, evaluate it using the framework's two-stage \
...
RC_GENERATOR_SYSTEM = """\
You are an SLA contract analyst specializing in automated renegotiation and remediation \
clauses, as performed by the Renegotiation Clause (RC) Generation step of the framework.

## Objective
Generate a single natural-language Renegotiation Clause (RC), expressed as a temporal amendment \
to the active SLA, based on:
...
- Describe the condition under which the clause is deactivated: the amendment is removed once \
the time-to-repair (TTR) period has elapsed or the violated metric has been restored to its \
agreed value, after which the original SLA conditions resume.
- Do not explain your reasoning.
- Do not mention negotiation rounds, offers, counteroffers, or bargaining behavior.
- Do not output JSON.
- Generate only the final clause.
- The final clause must contain all the metrics that have been modified (if new values were \
agreed for additional metrics, include them as well).
...
## Output Template
Upon occurrence of a **{violated_metric}** violation, **corrective adjustment** is activated to \
maintain **[agreed target]**. This temporal amendment remains in effect until the time to \
repair (TTR) is over or the value of the violated SLO is restored, after which normal SLA \
conditions resume and the clause is deactivated.
...
Profile produced by the Profile Generation step of an SLA renegotiation framework.
...
information.
3. Non-Fabrication: Rate 0-100. Absence of unsupported or hallucinated information; genuinely \
unspecified elements must be flagged rather than invented.
...
You are an expert LLM judge evaluating the quality of an SLA renegotiation process conducted by \
a game-theoretic and LLM-based bargaining framework.
...
latency should decrease), and whether the negotiation aims to remediate the breach or \
compensate proportionally.

2. ZOPA Compliance: Rate 0-100. Validity of intermediate offers and of the final agreement with \
respect to the Zone of Possible Agreement (ZOPA); proposals outside the ZOPA should be \
penalized.

3. Stakeholder Profile Alignment: Rate 0-100. Consistency between each agent's behaviour and \
its stakeholder profile — objectives, priorities, operational constraints, and tone. \
Inconsistencies or role-reversals should reduce the score.

4. Concession Strategy Coherence: Rate 0-100. Whether concessions are introduced gradually and \
logically justified, showing consistent movement toward agreement — early anchoring, gradual \
convergence, no erratic reversals, and appropriate tradeoffs across metrics.

5. Utility Consistency: Rate 0-100. Consistency between negotiation decisions and the computed \
